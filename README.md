...
계정 전환은 `Codex 정상 종료 요청 → 최대 5초 대기 → 남아 있는 Codex 패키지 프로세스 종료 → 약 1초 동안 완전 종료 상태 확인 → 인증 캐시 전환 → Windows 패키지 앱 활성화 → 실제 표시 창 확인` 순서로 처리됩니다. 실행 요청 후 프로세스만 생기고 표시 창이 30초 안에 열리지 않으면 자동 재시도를 반복하지 않고 직접 실행 안내를 표시합니다. 실행 중인 작업과 작성 중인 입력은 중단될 수 있으므로 계정을 누르기 전에 중요한 작업이 끝났는지 확인해야 합니다. 위젯 프로세스는 종료되지 않습니다.
