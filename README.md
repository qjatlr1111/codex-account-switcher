...
```powershell
./scripts/Build-Installer.ps1 -ApplicationVersion 1.1.0
```
...
계정 전환은 `Codex 정상 종료 요청 → 최대 5초 대기 → 남아 있는 Codex 패키지 프로세스 종료 → 인증 캐시 전환 → Codex 재실행` 순서로 처리됩니다. 실행 중인 작업과 작성 중인 입력은 중단될 수 있으므로 계정을 누르기 전에 중요한 작업이 끝났는지 확인해야 합니다. 위젯 프로세스는 종료되지 않습니다.
